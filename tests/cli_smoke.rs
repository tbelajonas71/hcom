...
#[test]
fn start_as_relocate_recovers_stopped_codex_snapshot_without_transcript_path() {
    let h = Hcom::new();
    let session_id = "thread-cli-relocate-transcriptless";
    let old_directory = h.workspace.join("old-scaffold-transcriptless");
    let new_directory = h.workspace.join("cultivation-transcriptless");
    fs::create_dir_all(&old_directory).unwrap();
    fs::create_dir_all(&new_directory).unwrap();
    let sessions = h.codex_home.join("sessions/2026/09/02");
    fs::create_dir_all(&sessions).unwrap();
    ...
    command
        ...
        .env("CODEX_SESSION_ID", session_id)
        .args(["start", "--as", "cultivation", "--relocate"]);
    let output = command.output().expect("run transcriptless relocation CLI");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(output.status.success(), "stdout={stdout} stderr={stderr}");
